(() => {
  ...
  function normalize(csvText) {
    ...
    if ([idx.day, idx.word, idx.pos, idx.meaning].some((v) => v < 0)) {
      throw new Error("시트 헤더는 Day / 영단어 / 품사 / 뜻 / 사용 순서여야 합니다.");
    }
  ...
  function getStats() {
    return JSON.parse(
      localStorage.getItem("loroStatsV31") ||
  ...
  function getSchedules() {
    return JSON.parse(localStorage.getItem("loroReviewSchedulesV31") || "[]");
  }

  function saveSchedules(value) {
    localStorage.setItem("loroReviewSchedulesV31", JSON.stringify(value));
  }
  ...
  function choosePool(poolType, reviewDay = null) {
    if (poolType === "wrong") {
      const saved = JSON.parse(localStorage.getItem("loroWrongV31") || "[]");
  ...
  function checkAnswer(button, choice, question, correct) {
    ...
    if (choice === correct) {
      ...
      const saved = new Set(
        JSON.parse(localStorage.getItem("loroWrongV31") || "[]")
      );
      saved.add(keyOf(question));
      localStorage.setItem("loroWrongV31", JSON.stringify([...saved]));
    }
  ...
  function finishQuiz() {
    ...
    localStorage.setItem("loroStatsV31", JSON.stringify(s));
  ...
  $("resetBtn").addEventListener("click", () => {
    if (confirm("학습 기록, 오답, 복습 일정을 모두 초기화할까요?")) {
      [
        "loroStatsV31",
        "loroWrongV31",
        "loroReviewSchedulesV31"
